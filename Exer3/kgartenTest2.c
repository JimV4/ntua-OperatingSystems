...
struct kgarten_struct {
         ...
         */
         ...
         pthread_cond_t check_children;
         pthread_cond_t check_teachers;
...
void child_enter(struct thread_info_struct *thr)
{
        ...
        while ((float)thr->kg->vt < ((float)(thr->kg->vc + 1) / (float)thr->kg->ratio)) {
            pthread_cond_wait(&thr->kg->check_children, &thr->kg->mutex_children);
        }
...
void child_exit(struct thread_info_struct *thr)
{
        ...
        if ((thr->kg->vt - 1) * thr->kg->ratio >= thr->kg->vc) {
            pthread_cond_broadcast(&thr->kg->check_teachers);
        }
        pthread_mutex_unlock(&thr->kg->mutex_children);
}
...
void teacher_enter(struct thread_info_struct *thr)
{
        ...
        ++(thr->kg->vt);
        pthread_mutex_unlock(&thr->kg->mutex);
        if ((float)thr->kg->vt >= ((float)(thr->kg->vc + 1) / (float)thr->kg->ratio)) {
            pthread_cond_broadcast(&thr->kg->check_children);
        }
...
void teacher_exit(struct thread_info_struct *thr)
{
        ...
        while ((thr->kg->vt - 1) * thr->kg->ratio < thr->kg->vc) {
            pthread_cond_wait(&thr->kg->check_teachers, &thr->kg->mutex_teachers);
        }
